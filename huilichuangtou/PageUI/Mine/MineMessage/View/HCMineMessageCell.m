...
@implementation HCMineMessageCell
...
- (void)setMineMessageModel:(HCMineMessageModel *)model indexPath:(NSIndexPath *)indexPath {
    ...
    [btnFunc addTouch:^{
        if([self.delegate respondsToSelector:@selector(HCMineMessageCellClick:indexPath:)]) {
            [self.delegate HCMineMessageCellClick:model indexPath:indexPath];
        }
    }];
